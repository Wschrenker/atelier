# -*- coding: utf-8 -*-
"""Erzeugt die Arbeitsansicht `09_zz_zz_01_pruefstellen_nach_seiten/` aus den Quellen.
...
def _wurzel_finden(start):
    """Repository-Wurzel ab dem Skriptpfad suchen, am `.git`-Verzeichnis erkannt.

    Robust gegenueber Verschiebungen des Skripts: es zaehlt nicht die
    Ordnertiefe, sondern steigt auf, bis es die Wurzel findet.
    """
    pfad = os.path.abspath(start)
    while True:
        ...
        if eltern == pfad:
            raise RuntimeError(
                "Repository-Wurzel (.git) nicht gefunden ab %s" % start)
        pfad = eltern


WURZEL = _wurzel_finden(os.path.dirname(os.path.abspath(__file__)))
...
ZIEL = os.path.join(WURZEL, "100_quellen", "10_hofenbitzer_band_1_digital",
                    "09_zz_zz_01_pruefstellen_nach_seiten")

FORMELN = "09_zz_05_prüfstelle_formel_zusammenzug"
TEXTE = "09_zz_06_prüfstelle_text_zusammenzug"
...
#
# Zwei Schreibweisen sind in Gebrauch: "**D7 — erledigt 2026-09-08**" in
# den eintrag-Quellen (D- und A/B/C-Punkte) und "**N20-erledigt — ...**"
# in der punkt-Quelle der ersten Zweitpruefung. Beide werden erkannt.
ERLEDIGT = re.compile(r"—\s*erledigt\b|\b[A-Za-z]+\d+-erledigt\b", re.I)
